...
def get_repos_name(wf_repo_name_list):
    ...
    wf_idx_list = [[], [], [], []]
    for i in range(0, 4):
        with open(f'../workflows_alert/wf_id/{i}.txt', 'r') as file:
...
if __name__ == '__main__':
    ...
    for i in range(0, 4):
        with open(f'../workflows_alert/wf_name/{i}.txt', 'w') as file:
    ...
    for i in range(0, 4):
        ...
        for repo_full_name in target_lists:
            try:
                ...
                base_dir = f'../workflows_alert/wf_tags/{i}'
    ...
    # 收集标签
    tags_freq_list = [{}, {}, {}, {}]
    for current_rank in range(0, 4):
        for filename in os.listdir(f"../workflows_alert/wf_tags/{current_rank}"):
            file_path = os.path.join(f"../workflows_alert/wf_tags/{current_rank}", filename)
    ...
    # 保存所有结果
    with open('../workflows_alert/wf_tags/tag_freq.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['alert', 'key', 'value'])
        for i in range(0, 4):
            data = tags_freq_list[i]
            for k, v in data.items():
                writer.writerow([i, k, v])
    with open('../workflows_alert/wf_tags/tag_freq_front.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['alert', 'key', 'value'])
        for i in range(0, 4):
